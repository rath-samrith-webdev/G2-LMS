...
if (isset($_SESSION['user']['uid'])) {
    ...
    if (isset($_POST['view'])) {
        ...
        if ($role_id == 1) {
            $query = $connection->prepare("SELECT * FROM leave_requests INNER JOIN persons ON persons.id == leave_requests.employee_id INNER JOIN leave_types ON leave_types.id = leave_requests.leave_type_id ORDER BY id DESC LIMIT 5");
        } else {
            $query = $connection->prepare("SELECT * FROM leave_requests INNER JOIN persons ON persons.id == leave_requests.employee_id WHERE employee_id=:uid ORDER BY id DESC LIMIT 5");
        }
